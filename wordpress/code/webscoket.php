...
class WebSocketServer implements MessageComponentInterface
{
    ...
    public function onOpen(ConnectionInterface $conn)
    {
        ...
        $connectionData->user_start_chat = null; // Default value or null
    ...
    public function onMessage(ConnectionInterface $from, $msg)
    {
        ...
        if ($this->clients->contains($from)) {
                ...
                if ($room_data == null) {
                    foreach ($this->clients as $client) {
                        ...
                        if ($clientData->room_guid == $senderData->room_guid && $clientData->user_id == $senderData->user_id) {
                            $client->send("{\"error\": \"session_finished\"}");
                        }
                    }
                    echo "room_data==null\n";
                    return;
            ...
            if ($data['type'] == "startCall") {
                $senderData->user_start_chat = 1;
            }
            // echo "DATA TYPE IS: " . $data['type'] . "\n";
            if ($senderData->user_start_chat == 0 &&  $senderData->is_model == 0) {
                // echo "USER NOT ENTERT CHAT 84";
                return;
            } else {
                // echo "GIRL In CHAT";
            }
            ...
            if ($count_how_many_same_user_in_room > 1) {
                ...
                $client->send(json_encode(['error' => 'moreThanOneUser']));
            ...
            if ($data['type'] == "update") {
                ...
                if ($sessionStatus == "1") {
                    ...
                    $response = $websocketHelpers->update_chat_time_use($room_data, $this->clients);
                    if ($count_how_many_total_members_in_room == 2 && $response->answer != 2) {
                        // echo '$response->answer   ' . $response->answer;
                        if ($response->answer == 1) {
                            // websocketHelpers::send_message($this->clients, $senderData, "{\"error\":\"finishTime30\"}", 2);
                        } else {
                            websocketHelpers::send_message($this->clients, $senderData, "{\"error\":\"finishTime\"}", 2);
                            $this->onClose($from);
                            return;
                        }
                    }
                    websocketHelpers::send_message($this->clients, $senderData, json_encode($response), 0);
                    // foreach ($this->clients as $client) {
                    //     // Retrieve stored roomData for this client
                    //     $clientData = $this->clients->offsetGet($client);
                    //     // Check if this client is in the same room and not the sender
    ...
    public function onError(ConnectionInterface $conn, \Exception $e)
    {
        ...
        websocketHelpers::writeToLog($errorMessage);

        // Optionally, you could echo the error message for immediate visibility,
        // but be cautious about revealing sensitive information in a production environment.
        echo $errorMessage . "\n";
